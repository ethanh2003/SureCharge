...
def cardSale(total, tax, discount, newWindow):
    ...
    for items in sale_items:
        item_list = item_list + "(" + str(items.product_id) + ") "
    sale_records.append(
        Sale(updateCheckNum(), datetime.now().date().strftime('%Y-%m-%d'),
             datetime.now().time().strftime('%H:%M:%S.%f'), item_list, currentUser.name, 'Card',
             total,
             tax, discount))
    saveData()
    readData()
    ...
    def runDrawer(E1):
        ...
        if answer:
            drawer_record.append(DrawerReport(drawer.startingTotal, drawer.CashOwed, drawer.cashSales, drawer.cardSales,
                                              ...
                                              datetime.now().time().strftime('%H:%M:%S.%f'), overShort))
